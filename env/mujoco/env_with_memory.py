...
class EnvWithMemory:
    ...
    def step(self, action, action_logits, value):
        if self.done:
            ...
            self.history_infos.append(dict(ep_return=self.ep_return))
            self.reset()
            if len(self.history_data_batches) >= self.min_return_chunk_num:
                data_batches = self.history_data_batches
                infos = self.history_infos
                self.history_data_batches = []
                self.history_infos = []
                return data_batches, infos, (self.obs, )
            else:
                return [], [], (self.obs, )
...
class DummyVecEnvWithMemory:
    ...
    def step(self, actions, action_logits, values):
        ...
        for i, env in enumerate(self.envs):
            cur_data_batches, cur_infos, model_input = env.step(actions[i], action_logits[i], values[i])
